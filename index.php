...
if (!isset($_SESSION['palabra'])) {
    $_SESSION['palabra'] = $palabras[array_rand($palabras)]; //   array_rand ->sirve para obtener una o más claves aleatorias de un array
    $_SESSION['vidas'] = 6; // Número máximo de vidas
    $_SESSION['letras_acertadas'] = str_repeat('?', strlen($_SESSION['palabra'])); // strlen() Significa “string length” (longitud de cadena). Sirve para saber cuántos caracteres tiene un texto. y 
    $_SESSION['letras_usadas'] = [];                                               // str_repeat() Significa “string repeat” (repetir cadena). Sirve para repetir un texto varias veces.
}

// Procesar la letra enviada
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['letra'])) {
    $letra = strtolower($_POST['letra']); // strtolower convierte todo el texto a minúsculas
    ...
    if (in_array($letra, $_SESSION['letras_usadas'])) {
        ...
        if (strpos($_SESSION['palabra'], $letra) !== false) { // strpos busca la posición (el lugar) donde aparece una subcadena dentro de una cadena.
